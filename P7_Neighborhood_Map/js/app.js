...
var ViewModel = function() {
  var self = this;
  this.map = ko.observable(new google.maps.Map($('#map')[0], {
  ...
  this.setCurrentLocation = function(data) {
      ...
      self.map().panTo(data.marker().position);
      foursquareQuery = {
        ll: data.marker().position.lat() + "," + data.marker().position.lng(), query: data.marker().title
      };
      /**
      Foursqure venue search API returns the list of venues without phone number
      and URL. This calls search API first to get venueId. Then, call venues API
      with the given venueId to get details.
      */
      ...
      $.ajax({
        ...
        $.ajax({
          url: fourSquareVenueUrl + "/" + res.response.venues[0].id,
          data: fourSquare
        }).done(function(res) {
